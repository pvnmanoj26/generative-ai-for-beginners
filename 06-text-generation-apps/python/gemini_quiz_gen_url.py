...
def embed_text(text, task_type="retrieval_document"):
    ...
    emb = genai.embed_content(model=model, content=text, task_type=task_type)
...
def home():
    if request.method == "POST":
        ...
        # Step 2: Build Quiz
        query_embedding = embed_text(topic or "study material", task_type="retrieval_query")
        D, I = index.search(np.array([query_embedding]).astype("float32"), k=5)
        relevant_chunks = [chunks[i] for i in I[0]]
        context = "\n\n".join(relevant_chunks)

        system_instruction = "You are a helpful teacher. Always respond with valid JSON."
        quiz_prompt = f"""
        ...
        """
        quiz_json = get_gemini_response(system_instruction, quiz_prompt)
